...
const mapNameFamily = (characters) => {
  ...
  let format = characters.map(
    (character) => `${character.name}, house of ${character.house}`
  );
...
const filterFamily = (characters, house) => {
  ...
  let houses = characters.filter((character) => character.house == house);
...
const reduceHouses = (characters) => {
  ...
  /* Short hand way shown in video, can be on one line
  const houses = characters.reduce((acc, curr) => {
